    ...
    $(document).ready(function(){
      ...
      tsmId ="<%=Constants.TEAM_LEADER%>";
         if(roleId == tsmId) {
           ...
           hubName = "<%=request.getSession().getAttribute(Constants.ASSIGNED_HUB_NAME)%>"; 
          $('#circlesListContainer').remove();
          $('#regionsListContainer').remove();
          $('#zonesListContainer').remove();
          $('#hubsListContainer').remove();
      	 } 
       ...
       // Removes all the rows in the table except the first
        ...
        <%-- 	var url = "<%=request.getContextPath()%>/js/jChartFX/data/attendance-data.json"; --%>
          ...
          requestData = JSON.stringify(requestData)
           ...
           title.setText("Runners Attendance");
           ...
           promise.done(function (responseData) {
               if(responseData.errorMessage != "undefined") {
                  ...
                  for(var i= 0; i<responseData.attendanceDetails.length;i++) {
                     var html = '<tr><td>'+responseData.attendanceDetails[i].date+'</td><td>'+responseData.attendanceDetails[i].noOfRunnersPresent+'</td><td>'+hubName+'</td></tr>';
           ...
           promise.fail(function (data) {
        	   requestData = {};
             console.log("unable to process request for chart data");
           });
        }
      });
...
<div id="form-content">
  <div id="titleheading">
    <div class="navtitle1"><bean:message key="label.reports.attendance"/></div>
    <br />
    <div id="titleheadingbar">
      <div class="navtitle"><i class="fa fa-clipboard"></i>  <bean:message key="label.reports"/> /  <label><bean:message key="label.reports.attendance"/></label></div>
      <div id="nav3">
        <%long roleId = 0L;
    ...
    <div class="divheading10">
     <bean:message key="label.reports.attendance"/></i>
    </div>
    <div class="reportContent innerAddBlock" id="panel5">
    ...
    <div id="reportcontent" class="hide-reports-table">
      <table id="attendance-table" width="100%" align="center" cellpadding="0" cellspacing="0"
        border="0" class="records-page">
        <tr class="reporttable">
          <th>Date<span> (mm/dd/yyyy)</span></th>
